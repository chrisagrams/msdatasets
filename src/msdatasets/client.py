...
async def _stream_pride_import(
    client: httpx.AsyncClient,
    result: PrideDatasetResponse,
    *,
    on_status: Callable[[str, PrideImportStatus], None] | None = None,
) -> None:
    """Stream SSE events for a PRIDE import until all jobs complete."""
    url = f"{get_api_url()}/pride/datasets/{result.dataset_id}/stream"
    job_states: dict[str, PrideImportStatus] = {}

    async with client.stream("GET", url) as response:
        if response.status_code != 200:
            raise DownloadError(
                f"SSE stream failed for PRIDE import: HTTP {response.status_code}"
            )
        async for event_type, data in _iter_sse_events(response):
            if event_type == "done":
                return

            status = PrideImportStatus(data["status"])
            file_name = data.get("pride_file_name", "unknown")

            if status == PrideImportStatus.FAILED:
                error = data.get("error_message") or "unknown error"
                raise DownloadError(
                    f"PRIDE import failed for {file_name}: {error}"
                )
            ...
            complete = sum(
                ...
                if s == PrideImportStatus.COMPLETE
            )
            log.info(
                "PRIDE import: %d/%d files ready",
                complete,
                len(job_states),
            )

            if all(
                s == PrideImportStatus.COMPLETE for s in job_states.values()
            ):
                return


async def trigger_pride_import(
    accession: str,
    *,
    filenames: list[str] | None = None,
    client: httpx.AsyncClient,
    ...
    """Trigger a PRIDE import and wait until all jobs complete.

    Posts to ``/pride/{accession}/dataset`` and streams SSE events
    until all import jobs reach a terminal state.

    Returns the final :class:`PrideDatasetResponse`.
    """
    url = f"{get_api_url()}/pride/{accession}/dataset"
    log.info("Triggering PRIDE import for %s", accession)

    body = PrideDatasetRequest(filenames=filenames)
    if filenames:
        log.info("Requesting specific files: %s", filenames)

    resp = await client.post(url, json=body.model_dump(exclude_none=True))
    if resp.status_code == 404:
        raise DatasetNotFoundError(f"PRIDE project not found: {accession}")
    if resp.status_code >= 400:
        raise DownloadError(
            f"Failed to trigger PRIDE import for {accession}: "
            f"HTTP {resp.status_code}"
        )

    result = PrideDatasetResponse.model_validate(resp.json())

    # Check if already done
    if all(j.status == PrideImportStatus.COMPLETE for j in result.jobs):
        log.info(
            "PRIDE import complete for %s (dataset %s)",
            accession,
            result.dataset_id,
        )
        return result

    failed = [j for j in result.jobs if j.status == PrideImportStatus.FAILED]
    if failed:
        details = ", ".join(
            f"{j.pride_file_name or j.job_id or 'unknown'}: "
            f"{j.error_message or 'unknown error'}"
            for j in failed
        )
        raise DownloadError(f"PRIDE import failed — {details}")

    await _stream_pride_import(client, result, on_status=on_status)
